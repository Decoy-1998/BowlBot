...
const MOD_NAME = "BowlBot"
  MOD_FULL_NAME = "Bowl Bot"
  MOD_VERSION = "1.0.0"
  PET_BOWL = "PetBowl";

// Register mod with Mod SDK
const bowlBot = bcModSDK.registerMod({
	name: MOD_NAME,
...
function onTrigger(target) {
  ...
  ChatRoomSendWhisper(target.MemberNumber, `${target.Name}, you know you're not allowed to feed yourself! Ask someone else to help you!`);
}
...
bowlBot.hookFunction("ChatRoomSyncItem", -5, (args, next) => {
  const [ data ] = args;
  const item = data?.Item;
  const target1 = getPlayer(data?.Source);
  const target2 = getPlayer(item?.Target);
  if (!target1 || !target2) return next(args);
  if (
    target1.IsPlayer() || !target2.IsOwnedByPlayer() ||
    item?.Name !== PET_BOWL ||
    InventoryGet(target2, "ItemDevices")?.Asset?.Name === PET_BOWL
  ) return next(args);
  next(args);
  onTrigger(target2);	
});